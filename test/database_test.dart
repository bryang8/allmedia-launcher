...
void main() {
  ...
  test("listApplications", () async {
    await database.customInsert("INSERT INTO apps(package_name, name, version, banner, icon)"
        " VALUES('com.allmedia.launcher', 'FLauncher', '1.0.0', null, null);");

    final apps = await database.listApplications();

    expect(apps.length, 1);
    expect(apps[0].packageName, "com.allmedia.launcher");
  ...
  test("persistApps", () async {
    ...
    await database.persistApps(
        [AppsCompanion.insert(packageName: "com.allmedia.launcher", name: "FLauncher 2", version: "1.1.0")]);

    final app = await database.customSelect("SELECT * FROM apps;").getSingle();
    expect(app.read<String>("package_name"), "com.allmedia.launcher");
  ...
  test("updateApp", () async {
    await database.customInsert("INSERT INTO apps(package_name, name, version, banner, icon, hidden)"
        " VALUES('com.allmedia.launcher', 'FLauncher', '1.0.0', null, null, false);");
    await database.updateApp("com.allmedia.launcher", AppsCompanion(hidden: Value(true)));
  ...
  test("deleteApps", () async {
    await database.customInsert("INSERT INTO apps(package_name, name, version, banner, icon)"
        " VALUES('com.allmedia.launcher', 'FLauncher', '1.0.0', null, null);");
    await database.customInsert("INSERT INTO apps(package_name, name, version, banner, icon)"
        " VALUES('com.allmedia.launcher.2', 'FLauncher 2', '1.0.0', null, null);");

    await database.deleteApps(["com.allmedia.launcher"]);

    final app = await database.customSelect("SELECT * FROM apps;").getSingle();
    expect(app.read<String>("package_name"), "com.allmedia.launcher.2");
  });
  ...
  test("deleteCategory", () async {
    ...
    await database.customInsert("INSERT INTO apps_categories(category_id, app_package_name, 'order')"
        " VALUES($categoryId, 'com.allmedia.launcher', 0);");
  ...
  test("deleteAppCategory", () async {
    ...
    await database.deleteAppCategory(categoryId, "com.allmedia.launcher");
  ...
  test("insertAppsCategories", () async {
    ...
    await database.insertAppsCategories([
      AppsCategoriesCompanion.insert(categoryId: categoryId, appPackageName: "com.allmedia.launcher", order: 0),
    ]);
    ...
    expect(appCategory.read<String>("app_package_name"), "com.allmedia.launcher");
  ...
  test("replaceAppsCategories", () async {
    ...
    await database.replaceAppsCategories(
        [AppsCategoriesCompanion.insert(categoryId: categoryId, appPackageName: "com.allmedia.launcher", order: 1)]);
  ...
  test("listCategoriesWithApps", () async {
    ...
    await database.customInsert("INSERT INTO apps(package_name, name, version, banner, icon)"
        " VALUES('com.allmedia.launcher.2', 'FLauncher 2', '1.0.0', null, null);");
    await database.customInsert("INSERT INTO apps(package_name, name, version, banner, icon, hidden)"
        " VALUES('com.allmedia.launcher.3', 'FLauncher 3', '1.0.0', null, null, true);");
    final categoryId = await database.customInsert("INSERT INTO categories(name, 'order') VALUES('Test', 2);");
    await database.customInsert("INSERT INTO apps_categories(category_id, app_package_name, 'order')"
        " VALUES($categoryId, 'com.allmedia.launcher', 1);");
    await database.customInsert("INSERT INTO apps_categories(category_id, app_package_name, 'order')"
        " VALUES($categoryId, 'com.allmedia.launcher.2', 0);");
    await database.customInsert("INSERT INTO apps_categories(category_id, app_package_name, 'order')"
        " VALUES($categoryId, 'com.allmedia.launcher.3', 2);");
    ...
    expect(categoriesWithApps[0].applications[0].packageName, "com.allmedia.launcher.2");
    expect(categoriesWithApps[0].applications[0].name, "FLauncher 2");
    expect(categoriesWithApps[0].applications[1].packageName, "com.allmedia.launcher");
